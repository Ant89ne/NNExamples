...
def getDailyData(csvreader):
    ...
    for i, text in enumerate(header) : 
        if text == "New_deaths":         #Number of cases
            indexCases = i
...
model = torch.load("myModelMultipleDeaths.pt")
allMetrics = []
...
fullTestMultipleLoop(mydataTest, model, nbInput, nbOutput, device)
exit()
...
torch.save(model, "myModelMultipleDeaths.pt")
