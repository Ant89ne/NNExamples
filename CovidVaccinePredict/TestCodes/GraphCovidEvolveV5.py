...
def getLabels(listValues, nbSteps):
    nbmaxC = np.max([nbSteps, np.max(listValues)])
...
def visualization(listCases, listDeaths, countryName) :
    ...
    plt.legend(["2020", "2021"])
    plt.title("Cumulative Cases per months in " + countryName)
    ...
    plt.legend(["2020", "2021"])
    plt.title("Cumulative Deaths per months in " + countryName)

    plt.savefig("VisFold5/" + countryName + "Graph.png")
    # plt.show()
